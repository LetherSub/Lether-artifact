...
approx_proof = 1
alpha4 = mp.sqrt((n_cols + m_rows) * d * (6.36*1.6)**2 )

alpha5 = mp.sqrt(num_rec * d * (12193.83*1.6)**2 + n_prime  * d * (m_rows*d/2)**2)
...
stdev5 = gamma5 * mp.sqrt(337) * alpha5
...
gamma5 = stdev5 / (mp.sqrt(337) * alpha5)
# XXX
...
while True:
    ...
    m2 = kmlwe + kmsis + l + lmbda/2 + 256/d + 1 + approx_proof * 256/d + 1 + add_ARP * (256/d + 1)
...
rate = M1 * M2 * M3 * ((1-approx_proof) + approx_proof*M4) * M5
...
short_bits2 = 256 * (ceil(log(stdev3, 2) + 2.5)) + \
    approx_proof * 256 * (ceil(log(stdev4, 2) + 2.5)) + add_ARP * 256 * (ceil(log(stdev5, 2) + 2.5))
...
if not (kmlwe >= 0 and kmlwe == m2 - kmsis - l - lmbda/2 - 256/d - approx_proof * 256/d - 2 - add_ARP * (256/d + 1)):
...
printv(
    f"stdev5 = {stdev5}, log(stdev4/1.55) = {mp.log(stdev5/mpf(1.55),2)}")
printv(f"")
...
log2stdev5 = int(mp.log(stdev5/mpf(1.55), 2))
...
{int_t(f"{name}_scM5", int(mp.nint(mpf(2^128) * M5)))}
...
{int_t(f"{name}_stdev5sq", int(mp.nint(stdev5^2)), 2*q_nlimbs)}
...
{int_t(f"{name}_Bz5", Bz5, 2*q_nlimbs)}
...
static const abdlop_params_t {name}_tbox = {{{{ {name}_ring, {name}_dcomp, {m1 + Z}, {m2}, {l}, {256/d * num_norm + 1 + add_ARP * (256/d + 1) + lmbda/2 + 1}, {kmsis}, {name}_Bsq, {nu}, {omega}, {omega_bits}, {eta}, {rejs1}, {log2stdev1}, {name}_scM1, {name}_stdev1sq, {rejs2}, {log2stdev2}, {name}_scM2, {name}_stdev2sq}}}};
static const abdlop_params_t {name}_quad_eval_ = {{{{ {name}_ring, {name}_dcomp, {m1 + Z}, {m2}, {l + 256/d * num_norm + 1 + add_ARP * (256/d + 1)}, {lmbda/2 + 1}, {kmsis}, {name}_Bsq, {nu}, {omega}, {omega_bits}, {eta}, {rejs1}, {log2stdev1}, {name}_scM1, {name}_stdev1sq, {rejs2}, {log2stdev2}, {name}_scM2, {name}_stdev2sq}}}};
