...
#include "../demos/Lether/params.h"
#include "../demos/Lether/data.h"
// extern const int num_rec;
// extern const int num_m;
// extern const int num_n;
extern polymat_t barC;
extern polyvec_t bar_ct, delat_h, delat_m;
extern poly_t sf_elem;
...
static void __lnp_tbox_compute_z345 (
    uint8_t hash[32], polyvec_t tB, polyvec_t z3, polyvec_t z4, polyvec_t z5, polyvec_t s1,
    ...
    const lnp_tbox_params_t params, const add_lnp_tbox_params_t add_param);
static int __lnp_tbox_check_z345 (uint8_t hash[32], polyvec_t z3, polyvec_t z4, polyvec_t z5,
                                 polyvec_t tB, const lnp_tbox_params_t params, const add_lnp_tbox_params_t add_param);
...
static void ___schwartz_zippel_accumulate_beta4 (
    ...
    spolyvec_t u2, spolymat_t t0, spolymat_t t1, spolymat_t t2);
static void
___schwartz_zippel_accumulate_beta5 (
    spolymat_ptr R2i[], spolyvec_ptr r1i[], poly_ptr r0i[],
    spolymat_ptr R2i2[], spolyvec_ptr r1i2[], poly_ptr r0i2[],
...
static void
___schwartz_zippel_accumulate_bits (
    spolymat_ptr R2i[], spolyvec_ptr r1i[], poly_ptr r0i[],
    spolymat_ptr R2i2[], spolyvec_ptr r1i2[], poly_ptr r0i2[],
    UNUSED spolymat_ptr R2t, spolyvec_ptr r1t, UNUSED poly_ptr r0t,
    const uint8_t seed[32], uint32_t dom, const lnp_tbox_params_t params,
    spolyvec_t u0, spolyvec_t u1, UNUSED spolyvec_t u2, spolymat_t t0,
    spolymat_t t1, UNUSED spolymat_t t2, const add_lnp_tbox_params_t add_param);    
...
static void ___schwartz_zippel_accumulate_l2 (
    ...
    spolyvec_t u0, spolyvec_t u1, spolyvec_t u2, spolymat_t t0, spolymat_t t1,
    spolymat_t t2);
static void ___schwartz_zippel_accumulate_range (
    spolymat_ptr R2i[], spolyvec_ptr r1i[], poly_ptr r0i[],
    spolymat_ptr R2i2[], spolyvec_ptr r1i2[], poly_ptr r0i2[],
    spolymat_ptr R2t, spolyvec_ptr r1t, poly_ptr r0t,
    const unsigned int *const *const Es, const unsigned int *Es_nrows,
    ...
    const uint8_t seed[32], uint32_t dom, const lnp_tbox_params_t params,
    spolyvec_t u0, spolyvec_t u1, spolyvec_t u2, spolymat_t t0, spolymat_t t1,
    spolymat_t t2, const add_lnp_tbox_params_srcptr add_param);    
static void ___schwartz_zippel_accumulate_z4 (
    ...
    const uint8_t seed[32], uint32_t dom, const lnp_tbox_params_t params,
    spolyvec_t u0, spolyvec_t u1, spolymat_t t0, spolymat_t t1, const add_lnp_tbox_params_t add_param);
static void ___schwartz_zippel_accumulate_z5 (
    spolymat_ptr R2i[], spolyvec_ptr r1i[], poly_ptr r0i[],
    spolymat_ptr R2i2[], spolyvec_ptr r1i2[], poly_ptr r0i2[],
    spolymat_ptr R2t, spolyvec_ptr r1t, poly_ptr r0t, polymat_t Ds,
    polymat_t Dm, polyvec_t u, polymat_t oDs, polymat_t oDm,polyvec_t z4,
...
_lnp_tbox_prove (uint8_t hash[32], polyvec_t tB, polyvec_t h, poly_t c,
                 ...
                 polyvec_t z4, polyvec_t z5, polyvec_t s1, polyvec_t m, polyvec_t s2,
                 ...
                 const lnp_tbox_params_t params, const add_lnp_tbox_params_t add_param)
{
  ...
  const unsigned int add_ARP = add_param->add_ARP;
  const unsigned int nRec = add_param->nRec;
  const unsigned int bit_proof = add_param->bit_proof;
  const unsigned int bit_length = add_param->bit_length;
  const unsigned int bit_index = add_param->bit_index;
  const unsigned int mes_index = add_param->mes_index;
  const unsigned int mp_index = add_param->mp_index;
  printf("nprime = %d\n", params->nprime);
  printf("nRec = %d\n", nRec);
  const unsigned int nprime = params->nprime - nRec;
  const unsigned int nex = params->nex;
  const unsigned int nbin = params->nbin;
  const unsigned int num_m = add_param->num_m;
  const unsigned int length_h = add_param->length_h;
  const unsigned int length_m = add_param->length_m;
  const unsigned int num_n = add_param->num_n;
  const unsigned int bin_h_index = add_param->bin_h_index;
  const unsigned int bin_m_index = add_param->bin_m_index;
  unsigned int n_equ;
  unsigned int i;
  spolymat_ptr R2prime_sz[lambda / 2 + 2*2 + bit_proof + nRec + 1], R2primei;
  spolyvec_ptr r1prime_sz[lambda / 2 + 2*2 + bit_proof + nRec + 1], r1primei;
  poly_ptr r0prime_sz[lambda / 2 + 2*2 + bit_proof + nRec + 1], r0primei;
  ...
  const unsigned int ibeta2 = (m1 + Z + l + loff + 256 / d + 1) * 2;
  ...
  polymat_alloc (barC, Rq, nRec, num_m);
  ...
  __lnp_tbox_compute_z345 (hash, tB, z3, z4, z5, s1, m, s2, Bprime, Es, Es_nrows,
  ...
  printf("finish __lnp_tbox_compute_z345\n");

  if(nprime > 0){
    polymat_t tmp_Ds;
    polyvec_t tmp_u;
    polymat_get_submat(tmp_Ds, Ds, 0, 0, nprime, m1, 1, 1);
    polyvec_get_subvec(tmp_u, u, 0, nprime, 1);

    if (Ds != NULL)
      {
        polymat_alloc (oDs, Rq, nprime, m1);
        polymat_auto (oDs, tmp_Ds);
      }
    if (l > 0 && Dm != NULL)
      {
  ...
  if( add_ARP > 0){
    polymat_t tmp_Ds;
    polyvec_t tmp_u;
    polymat_get_submat(tmp_Ds, Ds,  nprime, 0, nRec, m1, 1, 1);
    polyvec_get_subvec(tmp_u, u, nprime, nRec, 1);

    if (Ds != NULL)
      {
        polymat_alloc (oDs2, Rq, nRec, m1);
        polymat_auto (oDs2, tmp_Ds);
      }
    if (l > 0 && Dm != NULL)
      {
        polymat_alloc (oDm2, Rq, nRec, l);
        polymat_auto (oDm2, Dm);
      }
    if (u != NULL)
      {
        polyvec_alloc (ou2, Rq, nRec);
        polyvec_auto (ou2, tmp_u);
      }
  }
  ...
  polyvec_get_subvec (subv, s, (m1 + Z + l) * 2, loff + 1 + tloff, 2);
  polyvec_get_subvec (subv_auto, s, (m1 + Z + l) * 2 + 1, loff + 1 + tloff, 2);
  polyvec_get_subvec (subv2, m, l, loff + 1 + tloff, 1);
  ...
  if(add_ARP > 0){
    ...
    R2prime_sz[i] = R2primei;
    spolymat_set_empty (R2prime_sz[i]);
    poly = spolymat_insert_elem (R2prime_sz[i], ibeta2, ibeta2);
    poly_set_zero (poly);
    coeff = poly_get_coeff (poly, 0);
    int_set (coeff, params->inv4);
    poly = spolymat_insert_elem (R2prime_sz[i], ibeta2, ibeta2 + 1);
    poly_set_zero (poly);
    coeff = poly_get_coeff (poly, 0);
    int_set (coeff, Rq->inv2);
    poly = spolymat_insert_elem (R2prime_sz[i], ibeta2 + 1, ibeta2 + 1);
    poly_set_zero (poly);
    coeff = poly_get_coeff (poly, 0);
    int_set (coeff, params->inv4);
    R2prime_sz[i]->sorted = 1;
  ...
  // to prove the hamming weight of the bit string is 1, i.e., sum(b_i) = 1
  int k;
  if(bit_proof > 0)
  {
    for(k = 0; k < bit_proof; k++)
    {
      int j;
      i = lambda / 2 + 2 + 2 * add_ARP + k ;
      ...
      r1prime_sz[i] = r1primei;
      spolyvec_set_empty(r1prime_sz[i]);
      for(j = 0; j < bit_length / bit_proof; j++)
      {
        poly = spolyvec_insert_elem(r1prime_sz[i], bit_index*2 + j * 2);
        poly_set_zero (poly);
        coeff = poly_get_coeff (poly, 0);
        int_set_i64 (coeff, 1);
      }
      r1prime_sz[i]->sorted = 1;
  ...
  // (br_i - bs_i) * mp = m_i
  // br_i * mp - bs_i * mp - m_i = 0
  if(bit_proof > 0)
  {
    for(k = 0; k < nRec; k++)
    {
    
    i = lambda / 2 + 2 + 2 * add_ARP + bit_proof + k;
    R2primei = _alloc (sizeof (spolymat_t));
    spolymat_alloc (R2primei, Rq, np, np, NELEMS_DIAG (np));
    R2prime_sz[i] = R2primei;
    spolymat_set_empty (R2prime_sz[i]);

    poly = spolymat_insert_elem (R2prime_sz[i], bit_index*2 + nRec * 2 + k*2, mp_index*2);
    poly_set_zero (poly);
    coeff = poly_get_coeff (poly, 0);
    int_set_i64 (coeff, 1);

    poly = spolymat_insert_elem (R2prime_sz[i], bit_index*2 + k*2, mp_index*2);
    poly_set_zero (poly);
    coeff = poly_get_coeff (poly, 0);
    int_set_i64 (coeff, -1);
    ...
    poly = spolyvec_insert_elem(r1prime_sz[i], mes_index*2 + k * 2);
    poly_set_zero (poly);
    coeff = poly_get_coeff (poly, 0);
    int_set_i64 (coeff, -1);
    r1prime_sz[i]->sorted = 1;
  ...
  // verifiable decryption, sum(b_i * <bar_vec_c_i, sk> - b_i bar_ct = h + [q/2t+2] * m)
  if(bit_proof > 0)
  {
    i = lambda / 2 + 2 + 2 * add_ARP + bit_proof + nRec;
    R2primei = _alloc (sizeof (spolymat_t));
    spolymat_alloc (R2primei, Rq, np, np, NELEMS_DIAG (np));
    R2prime_sz[i] = R2primei;
    spolymat_set_empty (R2prime_sz[i]);
    ...
    for(j = 0; j < nRec; j++)
    {
      for(k = 0; k < num_m; k++)
      {
        poly = spolymat_insert_elem (R2prime_sz[i], bit_index*2 + j*2, (num_n + nRec)*2 + k*2);
        tmp2 = polymat_get_elem(barC, j, k);
        poly_set(poly, tmp2);
      }
    }
    R2prime_sz[i]->sorted = 1;
    ...
    {
      poly = spolyvec_insert_elem(r1prime_sz[i], bit_index*2 + j * 2);
      tmp2 = polyvec_get_elem(bar_ct, j);
      poly_set(poly, tmp2);
    }

    for(j = 0; j < length_h; j++)
    {
      poly = spolyvec_insert_elem(r1prime_sz[i], bin_h_index*2 + j * 2);
      tmp2 = polyvec_get_elem(delat_h, j);
      poly_set(poly, tmp2);
    }

    for(j = 0; j < length_m; j++)
    {
      poly = spolyvec_insert_elem(r1prime_sz[i], bin_m_index*2 + j * 2);
      tmp2 = polyvec_get_elem(delat_m, j);
      poly_set(poly, tmp2);
    }
  ...
  if (nprime > 0)
    {
      ...
      ___schwartz_zippel_accumulate_beta5 (
          R2prime_sz, r1prime_sz, r0prime_sz, R2prime_sz2, r1prime_sz2,
          r0prime_sz2, R2t, r1t, r0t, hash, lambda * 2 * (d - 1), params, u0, u1,
          u2, t0, t1, t2);
    }
  ...
  if (Z > 0)
    {
      ...
      ___schwartz_zippel_accumulate_upsilon (
          R2prime_sz, r1prime_sz, r0prime_sz, R2prime_sz2, r1prime_sz2,
          r0prime_sz2, R2t, r1t, r0t, hash, lambda * 3 * (d - 1), params, u0,
          u1, u2, t0, t1, t2);
    }
  ...
  if (nbin > 0)
    {
      ...
      ___schwartz_zippel_accumulate_bin (
          ...
          lambda * (3 * (d - 1) + 1), params, u0, u1, u2, t0, t1, t2);
    }
  ...
  ___schwartz_zippel_accumulate_range (
      R2prime_sz, r1prime_sz, r0prime_sz, R2prime_sz2, r1prime_sz2,
      r0prime_sz2, R2t, r1t, r0t, Es, Es_nrows, Em, Em_nrows, hash,
      lambda * (3 * (d - 1) + 2), params, u0, u1, u2, t0, t1, t2, add_param);
  ...
  if (nprime > 0)
    {
      ...
      ___schwartz_zippel_accumulate_z4 (
          ...
          lambda * (3 * (d - 1) + 2 + 1), params, u0, u1, t0, t1, add_param);
    }
  if (nprime > 0)
    {
      DEBUG_PRINTF (DEBUG_LEVEL >= 2, "%s", "schwarz-zippel z4 evalqs");
      ___schwartz_zippel_accumulate_z5 (
          R2prime_sz, r1prime_sz, r0prime_sz, R2prime_sz2, r1prime_sz2,
          r0prime_sz2, R2t, r1t, r0t, Ds, Dm, u, oDs2, oDm2, z5, hash0, // XXX why hash0 needed ??
          lambda * (3 * (d - 1) + 2 + 1 + 256), params, u0, u1, t0, t1, add_param);
    }
  ...
  if (nex > 0)
    {
      ...
      ___schwartz_zippel_accumulate_z3 (
          ...
          z3, hash0, lambda * (3 * (d - 1) + 2 + 1 + 256 * 2), params, u0, u1, t0,
          t1);
    }
 ...
 if (bit_proof > 0)
    {
      ...
      ___schwartz_zippel_accumulate_bits (
          R2prime_sz, r1prime_sz, r0prime_sz, R2prime_sz2, r1prime_sz2,
          r0prime_sz2, R2t, r1t, r0t, hash, lambda * (3 * (d - 1) + 2 + 1 + 256 * 3), params, u0, u1,
          u2, t0, t1, t2, add_param);
    }
...
_lnp_tbox_verify (uint8_t hash[32], polyvec_t h, poly_t c, polyvec_t z1,
                  polyvec_t z21, polyvec_t hint, polyvec_t z3, polyvec_t z4, polyvec_t z5,
                  ...
                  const unsigned int *Em_nrows, const unsigned int *const Ps,
                  const unsigned int Ps_nrows, polymat_t Ds, polymat_t Dm,
                  polyvec_t u, const lnp_tbox_params_t params, const add_lnp_tbox_params_t add_param)
{
  abdlop_params_srcptr tbox = params->tbox;
  abdlop_params_srcptr quad_eval = params->quad_eval->quad_eval;
  const unsigned int Z = params->Z;
  ...
  b = __lnp_tbox_check_z345 (hash, z3, z4, z5, tB, params, add_param);
  if (b != 1){
    printf("*******error 3 \n");
    goto ret;
  }
  ...
  if (nprime > 0)
    {
      ___schwartz_zippel_accumulate_z5 (
          R2prime_sz, r1prime_sz, r0prime_sz, R2prime_sz2, r1prime_sz2,
          r0prime_sz2, R2t, r1t, r0t, Ds, Dm, u, oDs2, oDm2, z5, hash0,
          lambda * (3 * (d - 1) + 2 + 1 + 256), params, u0, u1, t0, t1, add_param);
    }
...
static void
__lnp_tbox_compute_z345 (uint8_t hash[32], polyvec_t tB, polyvec_t z3,
                        polyvec_t z4, polyvec_t z5, polyvec_t s1, polyvec_t m, polyvec_t s2,
                        polymat_t Bprime, const unsigned int *const *const Es,
                        const unsigned int *Es_nrows,
                        const unsigned int *const *const Em,
                        ...
                        const lnp_tbox_params_t params, const add_lnp_tbox_params_t add_param)
{
  ...
  INTVEC_T (s5coeffs, nRec * d, int_get_nlimbs (q));
  INTVEC_T (y3coeffs, 256, int_get_nlimbs (q));
  INTVEC_T (y4coeffs, 256, int_get_nlimbs (q));
  INTVEC_T (y5coeffs, 256, int_get_nlimbs (q));
  INTVEC_T (z3coeffs, 256, int_get_nlimbs (q));
  INTVEC_T (z4coeffs, 256, int_get_nlimbs (q));
  INTVEC_T (z5coeffs, 256, int_get_nlimbs (q));
  polyvec_t tmp_polyvec, subv, s1_, m_, s21, y3_, y4_, y5_, ty3, ty4, ty5, tbeta, beta, tbeta2, beta2,
      upsilon, s3, s4, s5, y3, y4, y5, z3_, z4_, z5_;
  intvec_ptr coeffs;
  polymat_t By3, By4, By5, Bbeta, Bbeta2;
  shake128_state_t hstate;
  coder_state_t cstate;
  ...
  uint8_t out[CEIL (256 * 3 * log2q + 2 * d * log2q, 8) + 1];
  uint8_t cseed[32]; /* seed for challenge */
  poly_ptr poly, src, dst;
  int beta3 = 0, beta4 = 0, beta5 = 0;
  int rej;

  memset (out, 0, CEIL (256 * 3 * log2q + 2 * d * log2q, 8) + 1); // XXX


  polyvec_alloc (s3, Rq, nex);
  polyvec_alloc (s4, Rq, nprime);
  polyvec_alloc (s5, Rq, nRec);

  polyvec_alloc (y3, Rq, 256 / d);
  polyvec_alloc (y4, Rq, 256 / d);
  polyvec_alloc (y5, Rq, 256 / d);

  polyvec_alloc (z3_, Rq, 256 / d);
  polyvec_alloc (z4_, Rq, 256 / d);
  polyvec_alloc (z5_, Rq, 256 / d);

  polyvec_alloc (tmp_polyvec, Rq, MAX (nex, nprime));

  /* s1 = s1_,upsilon, m = m_,y3_,y4_,beta, y5_, beta2 */
  polyvec_get_subvec (s1_, s1, 0, m1, 1);
  if (l > 0)
    polyvec_get_subvec (m_, m, 0, l, 1);

  const unsigned int t_loff3 = (nex > 0 ? 256 / d : 0);
  const unsigned int t_loff4 = (nprime > 0 ? 256 / d : 0);
  const unsigned int t_loff = t_loff3 + t_loff4;

  polyvec_get_subvec (beta, m, l + t_loff, 1, 1);
  polyvec_set_zero (beta);

  polyvec_get_subvec (beta2, m, l + t_loff + 256 / d + 1, 1, 1);
  polyvec_set_zero (beta2);
  ...
  if (nprime > 0)
    {
      ...
      if (tmp_Ds != NULL)
        {
          polyvec_addmul (s4, tmp_Ds, s1_, 0);
          polyvec_fromcrt (s4);
        }
      if (Dm != NULL)
        {
  ...
  if(add_ARP > 0)
  {
      ...
      polyvec_get_subvec (y5_, m, l + loff, 256 / d, 1);

      polyvec_get_subvec (ty5, tB, l + loff, 256 / d, 1);
      polymat_get_submat (By5, Bprime, l + loff, 0, 256 / d, m2 - kmsis, 1, 1);

      loff += 256 / d;

      polyvec_set_coeffvec2 (s5, s5coeffs);
      polyvec_set_coeffvec2 (y5, y5coeffs);
      polyvec_set_coeffvec2 (z5_, z5coeffs);

      if (tmp_u != NULL)
        {
          polyvec_set (s5, tmp_u);
          polyvec_fromcrt (s5);
          polyvec_mod (s5, s5);
          polyvec_redc (s5, s5);
        }
      ...
      if (tmp_Ds != NULL)
        {
          polyvec_addmul (s5, tmp_Ds, s1_, 0);
          polyvec_fromcrt (s5);
        }
      if (Dm != NULL)
        {
          polyvec_addmul (s5, Dm, m_, 0);
          polyvec_fromcrt (s5);
        }

      polyvec_get_subvec (tbeta2, tB, l + loff, 1, 1);
      polymat_get_submat (Bbeta2, Bprime, l + loff, 0, 1, m2 - kmsis, 1, 1);
  }
  ...
  while (1)
    {
      ...
      if (add_ARP > 0)
        {
          /* y4, append to m  */
          polyvec_grandom (y5, add_param->log2stdev5, seed_tbox, dom++);
          polyvec_set (y5_, y5);

          polyvec_set (ty5, y5);
          polyvec_addmul (ty5, By5, s21, 0);
          polyvec_mod (ty5, ty5);
          polyvec_redp (ty5, ty5);

          beta5 = (rbits & (1 << (8 - nrbits + 1))) >> (8 - nrbits + 1);
          beta5 = 1 - 2 * beta5; /* {0,1} -> {1,-1} */
      ...
      if (nprime > 0){
        ...
        intvec_set_elem_i64 (coeffs, 0, beta4);
      }
      if (nex > 0)
        intvec_set_elem_i64 (coeffs, 0, beta3);

      if (add_ARP > 0){
      poly = polyvec_get_elem (beta2, 0);
      coeffs = poly_get_coeffvec (poly);

      intvec_set_elem_i64 (coeffs, d / 2, beta5);
      intvec_set_elem_i64 (coeffs, 0, beta5);
      }
      ...
      if (add_ARP > 0){
      polyvec_set (tbeta2, beta2);
      polyvec_addmul (tbeta2, Bbeta2, s21, 0);
      polyvec_mod (tbeta2, tbeta2);
      polyvec_redp (tbeta2, tbeta2);
      }
      ...
      if (add_ARP > 0)
        coder_enc_urandom3 (cstate, ty5, q, log2q);
      
      coder_enc_urandom3 (cstate, tbeta, q, log2q);
      if (add_ARP > 0)
      coder_enc_urandom3 (cstate, tbeta2, q, log2q);

      coder_enc_end (cstate);

      outlen = coder_get_offset (cstate);
      ASSERT_ERR (outlen % 8 == 0);
      ASSERT_ERR (outlen / 8 <= CEIL (256 * 3 * log2q + 2 * d * log2q, 8) + 1);
      outlen >>= 3; /* nbits to nbytes */
      ...
      if (nprime > 0)
      {
          ...
          int8_t Rprimei[nprime * d]; //TODO, to replace a new Rprimei for ARP2 for different nprime
    ...
    if(add_ARP > 0){
          INT_T (beta5Rprimeijs5j, int_get_nlimbs (q));
          int8_t Rprimei[nRec * d];
          int_ptr s5coeff, Rs5coeff;

          polyvec_fromcrt (s5);
          polyvec_fromcrt (y5);

          polyvec_set (z5_, y5);
          intvec_set_zero (y5coeffs);

          if (Rq->q->nlimbs == 1)
            {
              int64_t sRs5coeff[256];
              int64_t scoeff;
              int_ptr iptr;

              memset (sRs5coeff, 0, sizeof (sRs5coeff));

              for (i = 0; i < 256; i++)
                {
                  __expand_Rprime_i (Rprimei, nRec * d, i+256, cseed);
                  ...
                  for (j = 0; j < nRec * d; j++)
                    {
                      if (Rprimei[j] == 0)
                      ...
                      else
                        {
                          ...
                          s5coeff = intvec_get_elem (s5coeffs, j);
                          scoeff = int_get_i64 (s5coeff);

                          sRs5coeff[i] += Rprimei[j] * scoeff;
                        }
                    }
                }
              ...
              for (i = 0; i < 256; i++)
                {
                  iptr = intvec_get_elem (z5coeffs, i);
                  scoeff = int_get_i64 (iptr);
                  scoeff += beta5 * sRs5coeff[i];
                  int_set_i64 (iptr, scoeff);
                }
            }
          else
            {
              for (i = 0; i < 256; i++)
                {
                  Rs5coeff = intvec_get_elem (y5coeffs, i);
                  ...
                  for (j = 0; j < nRec * d; j++)
                    {
                      if (Rprimei[j] == 0)
                      ...
                      else
                        {
                          ...
                          int_set (beta5Rprimeijs5j, s5coeff);
                          int_mul_sgn_self (beta5Rprimeijs5j, Rprimei[j]);
                          int_add (Rs5coeff, Rs5coeff, beta5Rprimeijs5j);
                        }
                    }
                }

              intvec_mul_sgn_self (y5coeffs, beta5);
              intvec_add (z5coeffs, z5coeffs, y5coeffs);
            }
        }
      ...
      ASSERT_ERR (add_param->rej5 == 0 || add_param->rej5 == 2);
      if (add_ARP > 0 && add_param->rej5)
        {
          intvec_mul_sgn_self (y5coeffs, beta5); /* revert mul by beta4 */
          rej = rej_bimodal (rstate_rej, z5coeffs, y5coeffs, add_param->scM5,
                             add_param->stdev5sqr);
          if (rej)
            {
              DEBUG_PRINTF (DEBUG_PRINT_REJ, "%s", "reject s3");
...
static int
__lnp_tbox_check_z345 (uint8_t hash[32], polyvec_t z3, polyvec_t z4, polyvec_t z5,
                      polyvec_t tB, const lnp_tbox_params_t params, const add_lnp_tbox_params_t add_param)
{
  abdlop_params_srcptr tbox = params->tbox;
#if ASSERT == ASSERT_ENABLED
  ...
  INTVEC_T (z4coeffs, 256, int_get_nlimbs (q));
  unsigned int i;
  polyvec_t tmp_polyvec, ty3, ty4, ty5, tbeta, tbeta2;
  intvec_t isubv;
  poly_ptr poly;
  intvec_ptr coeffs;
  shake128_state_t hstate;
  ...
  if (nprime > 0)
    {
      ...
      polyvec_get_subvec (ty5, tB, l + loff + 256 / d + 1, 256 / d, 1);
      polyvec_mod (ty5, ty5);
      polyvec_redp (ty5, ty5);
  ...
  polyvec_get_subvec (tbeta2, tB, l + loff + 256 / d + 1, 1, 1);
  polyvec_mod (tbeta2, tbeta2);
  polyvec_redp (tbeta2, tbeta2);
  ...
  if (nprime > 0)
    {
      polyvec_fromcrt (z4);
      polyvec_l2sqr (l2sqr, z4);
      printf("l2sqr: ");
      int_dump(l2sqr);
      printf("Bz4: ");
      int_dump(params->Bz4);
      if (int_gt (l2sqr, params->Bz4)){
        printf("************ error-z 3.2\n");
        // goto ret;
      }
    }
    ...
    {  
      polyvec_fromcrt (z5);
      polyvec_l2sqr (l2sqr, z5);
      printf("l2sqr: ");
      int_dump(l2sqr);
      printf("Bz5: ");
      int_dump(add_param->Bz5);
      if (int_gt (l2sqr, add_param->Bz5)){
        printf("************ error-z 3.3\n");
        // goto ret;
      }
    ...
    spolymat_t t1, UNUSED spolymat_t t2)
{
  ...
  const unsigned int loff
      = (nprime > 0 ? 256 / d : 0) + (nex > 0 ? 256 / d : 0);
  // XXX 
  const unsigned int ibeta = (m1 + Z + l + loff + 256 / d + 1) * 2;
  poly_ptr poly;
  int_ptr coeffa, coeffb;
  unsigned int i, j;
  spolyvec_ptr r1tptr[1];
    ...
    spolymat_t t2, const add_lnp_tbox_params_srcptr add_param)
{
  ...
  spolyvec_ptr r1tptr[1];
  poly_ptr r0tptr[1];
  const int64_t sf_bal = add_param->sf_bal;
  const unsigned int bin_bal_index = add_param->bin_bal_index;
  const unsigned int bin_m_index = add_param->bin_m_index;
  const unsigned int length_m = add_param->length_m;
  const unsigned int balance_range = add_param->balance_range;
  const unsigned int beta_m = add_param->beta_m;
  // nelems = 0;
  // for (i = 0; i < Z; i++)
  //   nelems = MAX (nelems, ni[i]);
  ...
  coeff = poly_get_coeff (r0t, 0);
  int_set_i64 (coeff, sf_bal); /* <v,v> - Bi^2 */
  ...
  /* o(bin(Bi^2))*upsiloni */
  // printf("bin_m_index = %d \n", bin_m_index);
  for(i = 0; i < length_m; i++)
  {
    poly = spolyvec_insert_elem (r1t, 2 * (bin_m_index) + 2*i);
    poly_set_zero(poly);
    // printf("############### 7.1.%d\n", i);
    int64_t power2 = 1U << i;
    // printf("power2 = %ld \n", power2);
    if(i < length_m - 1)
    {
      for (j = 0; j < balance_range; j++)
      {
        int64_t power = 1U << j;
        coeff = poly_get_coeff (poly, j);
        int_set_i64 (coeff, power * power2);
    ...
    else
    {
      for (j = 0; j < balance_range; j++)
      {
        int64_t power = 1U << j;
        coeff = poly_get_coeff (poly, j);
        int_set_i64 (coeff, power * beta_m);
      }
    }
  ...
  poly = spolyvec_insert_elem (r1t, 2 * (bin_bal_index));
  poly_set_zero(poly);
    ...
    spolyvec_t u0, spolyvec_t u1, spolymat_t t0, spolymat_t t1, const add_lnp_tbox_params_t add_param)
{
  ...
  const unsigned int ibeta2 = (m1 + Z + l + loff + 256 / d + 1) * 2; // XXX correct
  const unsigned int is1 = 0;
  const unsigned int im = (m1 + Z) * 2;
  const unsigned int iy4 = (m1 + Z + l + loff3) * 2; // XXX correct
  const unsigned int iy5 = (m1 + Z + l + loff3 + 256 / d + 1) * 2; // XXX correct
  unsigned int i, j, k;
    ...
    spolyvec_t u0, spolyvec_t u1, spolymat_t t0, spolymat_t t1, const add_lnp_tbox_params_t add_param)
{
  ...
  const unsigned int nex = params->nex;
  const unsigned int nRec = add_param->nRec;
  const unsigned int nprime0 = params->nprime - nRec;
  const unsigned int nprime = nRec;
  const unsigned int loff3 = (nex > 0 ? 256 / d : 0);
  const unsigned int loff4 = (nprime > 0 ? 256 / d : 0);
  const unsigned int loff = loff3 + loff4;
  // const unsigned int ibeta = (m1 + Z + l + loff) * 2; // XXX correct
  const unsigned int ibeta = (m1 + Z + l + loff + 256 / d + 1) * 2; // XXX correct
  const unsigned int is1 = 0;
  const unsigned int im = (m1 + Z) * 2;
  // const unsigned int iy4 = (m1 + Z + l + loff3) * 2; // XXX correct
  const unsigned int iy4 = (m1 + Z + l + loff3 + 256 / d + 1) * 2; // XXX correct
  unsigned int i, j, k;
  int8_t Rprimei[nprime * d];
  const unsigned int lambda = params->quad_eval->lambda;
  spolymat_ptr R2tptr[1];
  ...
  polymat_t tmp_Ds;
  polyvec_t tmp_u;
  polymat_get_submat(tmp_Ds, Ds, nprime0, 0, nRec, m1, 1, 1);
  polyvec_get_subvec(tmp_u, u, nprime0, nRec, 1);
  // eval eqs in s1,o(s1),m,o(m),y4,o(y4),beta,o(beta) (approx. range proof
  // inf) prove z4 = y4 + beta4*Rprime*s4 over int vec of dim 256
  //       y4 - z4 + beta4*Rprime*s4 = 0
  //       y4 - z4 + beta4*Rprime*(Ds*s1+Dm*m+u) = 0  # view Ds resp Dm as int
  ...
  if (Rq->q->nlimbs == 1 && 1) // XXX add condition on q
    {
      ...
      for (i = 0; i < 256; i++)
        {
          __expand_Rprime_i (Rprimei, nprime * d, i + 256, seed);
          ...
          for (k = 0; k < lambda; k++)
            {
              ...
              // printf("!!!!!!!!!!!!!!! sch-Rprimer in z5:\n");
              // for (int i =0; i < nprime * d ;i++){
              //   printf("%d", Rprimei[i]);
              // }
              // printf("\n");
            ...
            const lnp_tbox_params_t params)
{
  ...
  polyvec_alloc (state->z5, Rq, 256 / d);
...
lnp_tbox_encproof (uint8_t *out, size_t *len, polyvec_t tA1, polyvec_t tB,
                   ...
                   polyvec_t hint, polyvec_t z3, polyvec_t z4, polyvec_t z5,
                   const lnp_tbox_params_t params, const add_lnp_tbox_params_t add_param)
{
  polyring_srcptr Rq = params->tbox->ring;
  int_srcptr q = Rq->q;
  ...
  coder_enc_grandom3 (cstate, z5, add_param->log2stdev5);
                   ...
                   const lnp_tbox_params_t params, const add_lnp_tbox_params_t add_param)
{
  ...
  coder_dec_grandom3 (cstate, z5, add_param->log2stdev5);
...
_lin_prover_prove (lin_prover_state_t state__, uint8_t *proof, size_t *len,
                   const uint8_t seed[32], const add_lnp_tbox_params_t add_param)
{
...
#endif
  // printf("before generate proof, u:\n");
  // polyvec_dump(state->u);
  /*
  ...
  _lnp_tbox_prove (hash, state->tB, state->h, state->c, state->z1, state->z21,
                   state->hint, state->z3, state->z4, state->z5, state_->s1, state_->m,
                   ...
                   seedsubproto, params, add_param);
  
  printf("_lnp_tbox_prove finish\n");
  ...
  lnp_tbox_encproof (proof, &prooflen, state->tA1, state->tB, state->h,
                     ...
                     state->z4, state->z5, params, add_param);
  if (len != NULL)
    *len = prooflen;
...
_lin_verifier_verify (lin_verifier_state_t state__, const uint8_t *proof,
                      size_t *len, add_lnp_tbox_params_t add_param)
{
  lin_params_srcptr linparam = state__->state->params;
  ...
  b = lnp_tbox_decproof (&prooflen, proof, state->tA1, state->tB, state->h,
                         ...
                         state->z3, state->z4, state->z5, params, add_param);
  if (len != NULL)
    *len = prooflen;
  if (b != 1){
    printf("*******error 1 \n");
  ...
  polyvec_mod (state->z5, state->z5);
  polyvec_redc (state->z5, state->z5);
  ...
  b = _lnp_tbox_verify (hash, state->h, state->c, state->z1, state->z21,
                        state->hint, state->z3, state->z4, state->z5, state->tA1,
                        ...
                        state->Ds, state->Dm, state->u, params, add_param);
  if (b != 1){
    printf("*******error TWO \n");
    goto ret;
  }
                  ...
                  const uint8_t coins[32])
{
  ...
  add_lnp_tbox_params_srcptr add_param = lin_state->add_param;
  ...
  _lnp_prover_set_statement_arp (__lnp_state, lin_state->Ds, lin_state->Dm,
                                 lin_state->u);
  _lin_prover_prove (state, proof, len, coins != NULL ? coins : coins_, add_param);
                     ...
                     size_t *len)
{
  ...
  _lnp_verifier_set_statement_arp (__lnp_state, lin_state->Ds, lin_state->Dm,
                                   lin_state->u);
  rc = _lin_verifier_verify (state, proof, len, add_param);
  DEBUG_PRINTF (DEBUG_PRINT_FUNCTION_RETURN, "%s end", __func__);
  return rc;
}
